...
class db_task extends  db_handler{
    ...
    public function getArrayProgetti($ricerca){
        ...
        $sql = "SELECT DISTINCT ID_Progetto, NomeP, DescrizioneP, DataScadenzaP, DataCreazioneP FROM progetto LEFT OUTER JOIN associatag on id = progetto WHERE ricercabile=1 AND (nome LIKE '%".$ricerca."%' OR tag LIKE '%".$ricerca."%') ORDER BY data_creazione DESC";
